...
void Game_functions::run(){
    ...
    if (calc_Leona){
        ...
        case 9:
            ...
            break;
        case 12:
            volante(&selec_robot.r3, 2, &vels[2]);
            break;
            //Fim Novas Funções
        case 100:
...
void Game_functions::killer_cpu(Robot *robo, int num_Robo, pair<float, float> *vels)
{
    ...
    //Corrige Posicionamento para o calculo do angulo entre a bola e o gol
    ball_pos.y = -ball_pos.y;
    centroid_atk.y=-centroid_atk.y;
    robo_pos.y = -robo_pos.y;
    ...
    //Calcula angulo entre o robô e a bola
    Point2d vec_robot_ball = ball_pos - robo_pos;
    double ang_robot_ball = angle_two_points(vec_robot_ball,eixo_x);
    if (vec_robot_ball.y < 0)
        ang_robot_ball = -ang_robot_ball;
    //ajusta angulos para menores que 180 e maiores que -180
    if (ang_robot_ball > 180)
        ang_robot_ball = ang_robot_ball - 360;
    else if (ang_robot_ball < -180)
        ang_robot_ball = ang_robot_ball + 360;
    // cout << "Angulo robo bola: " << ang_robot_ball << endl;
    ...
    // Limitando o Killer na defesa

    Point2d meiuca;
    meiuca.x = 90;
    meiuca.y = 70;
    //    if(centroid_atk.x > centroid_def.x){
    //        if(ball_pos.x > centroid_def.x + 70)
    //            meta = ball_pos;
    //        else{
    //            meta = meiuca;
    //        }
...
//            }
       ...
       //Se a bola estiver longe da área, defender sobre a linha das cruzes
        ...
        //Se o robô estiver na faixa do follow ball, parar de ir para a posição e começar a follow
                if(robo_pos.x<cx+10 && robo_pos.x>100)
                flag_return=0;
 ...
 //Variável que determina onde o robô irá se posicionar em x
        int cx = 50;
        //Se a flag estiver ativa, ele entra na return 2 defense
        //Se a bola estiver à esqueda da área, defender em cima da linha
            if(ball_pos.y < centroid_def.y - 35  &&  ball_pos.x <= cx){
                //meta.x = centroid_def.x - 8;
                //meta.y = centroid_def.y - 45;
                meta.x = 17;
       ...
       //Se a bola estiver longe da área, defender sobre a linha das cruzes
                ...
                flag_return = 1;
        //Se o robô estiver na faixa do follow ball, parar de ir para a posição e começar a follow
                if(robo_pos.x > cx-10 && robo_pos.x < 60)
                flag_return=0;
        //Se o robô estiver a frente da bola(com uma variável de folga para retornar mesmo na linha da bola
                if(robo_pos.x > ball_pos.x-5){
                meta.x = cx;
        ...
        //        set_g_size(meta_defender_root,robo);
...
void Game_functions::fake9(Robot *robo, int num_Robo, pair<float, float> *vels)
{
    ...
    if(ball_pos.y > (centroid_atk.y + 30))
    {
        meta.y = centroid_atk.y + 30;
    }
    else if(ball_pos.y < (centroid_atk.y - 30))
    {
        meta.y = centroid_atk.y - 30;
    }
    else
