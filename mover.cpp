...
double v_max_gol = 1.25;  //0.5
double v_delta_gol = 0.3;
double v_max_gol_ef = 1.5;
double w_max_gol = 5;
double kgol = (w_max_gol/v_max_gol);
double dist_giro_gol = 10.0;
double vel_giro_gol = 1.0;
double v_max_return = 1.25;
float kp_goalkepper = 8;

// Constantes para o zagueiro
double k_prevision = 1.5;
double v_max_prevision = 2.5;
float kp_defender = 8;

// Constantes para a movimentação com PID
double last_phi = 0; // Última orientação do robô
double lalpha_killer = 0; // Última orientação do killer
double last_theta_control = 0;
double ka = 0.15;
double lalpha_fake9 = 0;
double lalpha_volante = 0;
double lalpha_defender_root = 0;
double lalpha_guerreiro = 0;
...
void Mover::calcula_velocidades(Robot *r, Game_functions *potencial_fields, pair<float, float> *vels)
{
    ...
    case 4:
        ...
        break;
    case 8:
        velocity_guerreiro(r, potencial_fields , vels);
        break;
    case 9:
    ...
    case 21:
        ...
        break;
    case 12:
        velocity_volante(r, potencial_fields , vels);
        break;
    case 100:
...
void Mover:: velocity_goalkeeper(Robot *robo, Game_functions *pot_fields, pair<float, float> *vels)
{
    ...
    if (centroid_def.x <= centroid_atk.x)
    {
        ...
        if((ball_pos.x >= centroid_def.x + 30) && (robot_pos.x >= centroid_def.x + 30))
            v_max_return = 0.7;
        else
            v_max_return = 0.4;
        ...
        if (ball_v.x < 0 && (aux_position_y > centroid_def.y-25) && (aux_position_y < centroid_def.y+25))
        {
            if (robot_angle > 0)
            {
                v = -k_prevision*(aux_position_y-robot_pos.y)/tempo;
                if(v > v_max_prevision)
                {
                    v = v_max_prevision;
                }
                else if (v < -v_max_prevision)
                {
                    v = -v_max_prevision;
                }
                vels->first = v - w*l;
        ...
        else
        {
            if (ball_pos.x < 90 && ball_pos.y > 45 && ball_pos.y < 95 && robot_pos.x < centroid_def.x+20)
            {
                ...
                if (fabs(alpha) <= limiar_theta)
                {
                    w = kp_goalkepper*alpha/180 + kd*(alpha - last_phi);
                    limiar_theta = 90 - delta_limiar;
                }
                else
                {
            ...
            else
            {
                ...
                {
                    //v = v_max;
                    v = -v_delta_gol*fabs(alpha)/limiar_theta + v_max_return;
                    w = kp*alpha/180 + kd*(alpha - last_phi);
                    limiar_theta = 90 - delta_limiar;
                    ////cout << "Frente" << endl;
                }
                ...
                vels->second = v + w*l;
                //AdjustRobo
                if((fabs(robo->get_pos().x - pot_fields->get_meta_goalkeeper().x) < 5) && (fabs(robo->get_pos().y - pot_fields->get_meta_goalkeeper().y) < 5) &&(fabs(robot_angle) > 85) && (fabs(robot_angle) < 95)){
                    vels->first = 0;
                    vels->second = 0;
                }
                else if ((fabs(robo->get_pos().x - pot_fields->get_meta_goalkeeper().x) < 5) && (fabs(robo->get_pos().y - pot_fields->get_meta_goalkeeper().y) < 5)){
...
void Mover::velocity_killer_cpu(Robot *robo, Game_functions *pot_fields, pair<float, float> *vels)
{
    ...
    /*if (fabs(alpha) <= limiar_theta ){
        v = -v_delta*fabs(alpha)/limiar_theta + v_max;
        w = kp*alpha/180  + kd*(alpha-lalpha_killer);
        limiar_theta = 90 - delta_limiar;
    }
    else{
        alpha = ajusta_angulo(alpha+180);
    ...
    if (euclidean_dist(robo_pos, ball_pos) >= 25){
        if (killer_direction){
            ...
            //w = kp*alpha/180 + kd*(alpha-lalpha_killer);
            w = kp*alpha/180 + kd*(alpha-lalpha_killer);
            //            w = 7*alpha/180  + 0.0015*(theta-last_theta_control) + 0*(alpha - lalpha_killer);
            limiar_theta = 90 - delta_limiar;
            //cout << "1" << endl;
        }
        else{
        ...
        if((fabs(diff_angle < 60) || fabs(diff_angle > 120)) && ((centroid_def.x < centroid_atk.x && robo_pos.x < ball_pos.x) || (centroid_def.x > centroid_atk.x && robo_pos.x > ball_pos.x)))
        {
            if (fabs(alpha) <= limiar_theta ){
                v = -v_delta*fabs(alpha)/limiar_theta + v_max;
                w = 4*alpha/180 + kd*(alpha-lalpha_killer) + 2.5*diff_angle/180;;
                //            w = 7*alpha/180  + 0.0015*(theta-last_theta_control) + 0*(alpha - lalpha_killer);
                limiar_theta = 90 - delta_limiar;
                //cout << "3" << endl;
            }
    ...
    //cout << "atual a " << alpha << endl;
    //cout << "anterior a " << lalpha_killer << endl;
    //cout << "diferenca " << alpha-lalpha_killer << endl << endl;
    ...
    cout << "direita: " << v-w*l << endl;
    cout << "esquerda: " << v+w*l << endl << endl;

    if((pot_fields->get_meta_cpu()  != ball_pos) && (euclidean_dist(robo_pos, pot_fields->get_meta_cpu()) <= 10))
    {
        vels->first = 0;
        vels->second = 0;
    }
...
void Mover::velocity_defender_root(Robot *robo, Game_functions *pot_fields, pair<float, float> *vels){
    ...
    alpha = 90 - robot_angle - (line_root_defender-robot_pos.x)*k_angulo;
    alpha = ajusta_angulo(alpha);


    //PID
    if (fabs(alpha) <= limiar_theta ){
        w = kp*alpha/180 + kd*(alpha - lalpha_defender_root);
        limiar_theta = 90 - delta_limiar;
    }
    else{
        alpha = ajusta_angulo(alpha+180);
    ...
    if (centroid_def.x < centroid_atk.x){
        ...
        //PID
        if (fabs(alpha) <= limiar_theta ){
            w = kp_defender*alpha/180 + kd*(alpha - lalpha_defender_root);
            limiar_theta = 90 - delta_limiar;
        }
        else{
            alpha = ajusta_angulo(alpha+180);
        ...
        if (ball_v.x < 0 && ball_pos.x > centroid_def.x + line_root_defender){
            ...
            //                    //w = kp*alpha/180 + kd*(alpha-lalpha_defender_root);
            //                    w = kp*alpha/180 + kd*(alpha-lalpha_defender_root);
            //                    //            w = 7*alpha/180  + 0.0015*(theta-last_theta_control) + 0*(alpha - lalpha_defender_root);
            //                    limiar_theta = 90 - delta_limiar;
            //                    //cout << "1" << endl;
            //                }
            //                else{
    ...
    //PID
    if (fabs(alpha) <= limiar_theta ){
        w = 11*alpha/180 + 0.0*(alpha - lalpha_defender_root);
        limiar_theta = 90 - delta_limiar;
    }
    else{
        alpha = ajusta_angulo(alpha+180);
    ...
    //                //PID
    //                if (fabs(alpha) <= limiar_theta ){
    //                    w = 13*alpha/180 + kd*(alpha - lalpha_defender_root);
    //                    limiar_theta = 90 - delta_limiar;
    //                }
    //                else{
    //                    alpha = ajusta_angulo(alpha+180);
    ...
    //    cout << endl << "-----Cachaco velocity defender root-----" << endl;
    //    cout << robo->get_pos() << endl;
    //    cout << "-----Cachaco-----" << endl << endl;
...
void Mover::velocity_volante(Robot *robo, Game_functions *pot_fields, pair<float, float> *vels){

    Point2d meta = pot_fields->get_meta_volante();
    Point2d robot_pos = robo->get_pos();
    Point robot_grid = convert_C_to_G(robot_pos);
    double robot_angle = ajusta_angulo(robo->get_angle());
    ...
    //    cout << endl << "-----Cachaco velocity volante-----" << endl;
    //    cout << robo->get_pos() << endl;
    //    cout << "-----Cachaco-----" << endl << endl;
    ...
    if (centroid_def.x < centroid_atk.x){
        ...
        //PID
        if (fabs(alpha) <= limiar_theta ){
            w = kp_defender*alpha/180 + kd*(alpha - lalpha_volante);
            limiar_theta = 90 - delta_limiar;
        }
        else{
            alpha = ajusta_angulo(alpha+180);
        ...
        if (ball_v.x < 0 && ball_pos.x > centroid_def.x + line_volante){
            if (robot_angle > 0){
                v = -k_prevision*(aux_position_y-robot_pos.y)/tempo;
                if(v > v_max_prevision){
                    v = v_max_prevision;
        ...
        // ---------------------------- FollowBall ------------------------------//
        else if (ball_pos.x > centroid_def.x + line_volante && robo->get_pos().x < centroid_def.x + line_volante + 10 && robo->get_pos().x > centroid_def.x + line_volante - 10){
        ...
        // --------------------------- Return2Goal ----------------------------//
            ...
            if (fabs(alpha) <= limiar_theta ){
                //v = 0.4;
                v = -v_delta*fabs(alpha)/limiar_theta + 0.4;
                w = kp*alpha/180 + kd*(alpha - lalpha_volante);
                limiar_theta = 90 - delta_limiar;
                ////cout << "Frente" << endl;
            }
            else{
            ...
            //AdjustRobo
            if((euclidean_dist(robot_pos, pot_fields->get_meta_defender_root()) < 6) && (fabs(robo->get_angle()) > 85) && (fabs(robo->get_angle()) < 95)){
                vels->first = 0;
                vels->second = 0;
            }
            else if (euclidean_dist(robot_pos, pot_fields->get_meta_defender_root()) < 6){
                goalkeeper_orientation(robo,vels);
...
void Mover::velocity_fake9(Robot *robo, Game_functions *pot_fields, pair<float, float> *vels)
{
    ...
    //Restricao de posicao
    if (distancia_obst < distancia_permitida){
        if(fake9_direction && distancia_obst < distancia_permitida && fabs(another_ang) < 40){
            if (cont_fake9_obst > 5){
                fake9_direction = false;
                cont_fake9_obst = 0;
                cont_fake9_tempo = 0;
            }
            else{
                cont_fake9_obst = cont_fake9_obst + 1;
            }
        }
        else if(!fake9_direction && distancia_obst < distancia_permitida && fabs(another_ang) > 180 - 40){
            if (cont_fake9_obst > 5){
                fake9_direction = true;
                cont_fake9_obst = 0;
                cont_fake9_tempo = 0;
            }
            else{
        ...
        cont_fake9_obst = cont_fake9_obst - 1;
        if (cont_fake9_obst < 0){
            cont_fake9_obst = 0;
        }
    }

    //Restricao de tempo
    if(fabs(robo->get_pos().x - last_fake9.x) < 0.05 && fabs(robo->get_pos().y - last_fake9.y) < 0.05){
        cont_fake9_tempo = cont_fake9_tempo + 1;
        if (cont_fake9_tempo > 15){
            if(fake9_direction){
                fake9_direction = false;
            }
            else if(!fake9_direction){
                fake9_direction = true;
            }
        ...
        cont_fake9_tempo = cont_fake9_tempo - 1;
        if (cont_fake9_tempo < 0){
            cont_fake9_tempo = 0;
        }
    }
    //    cout << "COntador tempo " << cont_fake9_tempo << endl;
    last_fake9 = robo->get_pos();
    ...
    //    if (fabs(alpha) <= limiar_theta ){
    //         v = -v_delta*fabs(alpha)/limiar_theta + v_max;
    //         w = kp*alpha/180 + kd*(alpha-lalpha_fake9);
    //         //            w = 7*alpha/180  + 0.0015*(theta-last_theta_control) + 0*(alpha - lalpha_fake9);
    //         limiar_theta = 90 - delta_limiar;
    //         //cout << "3" << endl;
    //    }
    //    else{
    ...
    //Fim PD
    float ang_ball = vector_angle(robo->get_pos(),ball_pos);
    //cout << "ang_b: " << ang_ball << endl;
    if(euclidean_dist(robo->get_pos(),meta) < 6){
        v = 0;
        alpha = ang_ball - robo->get_angle();
        alpha = ajusta_angulo(alpha);
        if (fabs(alpha) <= limiar_theta){
            //w = k*v_max*alpha/180;

            w = 15*alpha/180 + kd*(alpha - last_phi);
        }
        else{
            alpha = ajusta_angulo(alpha+180);
            //w = k*v_max*alpha/180;
        ...
        //        if(fabs(ajusta_angulo(robo->get_angle()) - vector_angle(meta, robo->get_pos()) ) >= 20 )
        //        {
        //            v = 0;
        //            alpha = ajusta_angulo(vector_angle(meta, robo->get_pos()) - robo->get_angle());
        //            cout << "ang: " << alpha << endl;
        //            if (fabs(alpha) <= limiar_theta ){
        //                w = 2*kp*alpha/180;// + kd*(alpha-lalpha_killer);
        //                limiar_theta = 90 - delta_limiar;
        //            }
        //            else{
        //                alpha = ajusta_angulo(alpha+180);
    ...
    //cout << "Velocidade 1: " << vels->first << endl;
    //cout << "Velocidade 2: " << vels->second << endl;
}

void Mover::velocity_guerreiro(Robot *robo, Game_functions *pot_fields, pair<float, float> *vels){

    Point2d meta = pot_fields->get_meta_guerreiro();
    Point2d robot_pos = robo->get_pos();
    Point robot_grid = convert_C_to_G(robot_pos);
    double robot_angle = ajusta_angulo(robo->get_angle());
    ...
    if (centroid_def.x < centroid_atk.x){
        ...
        //PID
        if (fabs(alpha) <= limiar_theta ){
            w = kp_defender*alpha/180 + kd*(alpha - lalpha_guerreiro);
            limiar_theta = 90 - delta_limiar;
        }
        else{
            alpha = ajusta_angulo(alpha+180);
        ...
        if (ball_v.x < 0 && ball_pos.x > centroid_def.x + line_guerreiro){
            if (robot_angle > 0){
                v = -k_prevision*(aux_position_y-robot_pos.y)/tempo;
                if(v > v_max_prevision){
                    v = v_max_prevision;
        ...
        // ---------------------------- FollowBall ------------------------------//
        else if (ball_pos.x > centroid_def.x + line_guerreiro && robo->get_pos().x < centroid_def.x + line_guerreiro + 10 && robo->get_pos().x > centroid_def.x + line_guerreiro - 10){
        ...
        // --------------------------- Return2Goal ----------------------------//
            ...
            if (fabs(alpha) <= limiar_theta ){
                //v = 0.4;
                v = -v_delta*fabs(alpha)/limiar_theta + 0.4;
                w = kp*alpha/180 + kd*(alpha - lalpha_guerreiro);
                limiar_theta = 90 - delta_limiar;
                ////cout << "Frente" << endl;
            }
            else{
